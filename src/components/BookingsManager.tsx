...
import { useBookings, useOrganizations, venueCodes, type BookingRow } from "@/lib/data";
import {
  cancelBookingFn,
  deleteBookingFn,
  restoreBookingFn,
...
export function BookingsManager({
  ...
  const rows = useMemo(() => {
    return bookings
      .filter((b) => (scope === "own" ? b.organization_id === organizationId : true))
      .filter((b) => (orgFilter === "all" ? true : b.organization_id === orgFilter))
      .filter((b) => (tab === "upcoming" ? b.date >= today : b.date < today))
      .sort((a, b) =>
        tab === "upcoming"
  ...
  async function handleCancel(booking: BookingRow) {
    if (!window.confirm(`Cancel this booking? The venue will be freed immediately.`)) return;
    try {
    ...
    try {
      ...
      if (!result.ok) {
        const list = (result.conflicts ?? [])
          .map((c) => `${c.venue_code} — ${c.org_abbr} ${c.start_time}–${c.end_time}`)
          .join("; ");
      ...
      toast.error(error instanceof Error ? error.message : "Could not restore booking.");
    }
  }
  ...
  async function handleFlag(
    ...
    try {
      await setFlags({ data: { bookingId: booking.id, [field]: value } });
      await queryClient.invalidateQueries({ queryKey: ["bookings"] });
    } catch (error) {
    ...
    <Card>
      <CardHeader className="gap-4 sm:flex-row sm:items-center sm:justify-between">
        <CardTitle className="text-base">
          {scope === "own" ? "My bookings" : "All bookings across campus"}
        </CardTitle>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="text-center">Event done</TableHead>
                <TableHead className="text-center">Permission signed</TableHead>
                  ...
                  <TableRow key={b.id}>
                    ...
                    <TableCell className="text-center">
                      <Checkbox
                        ...
                        onCheckedChange={(v) => handleFlag(b, "eventDone", v === true)}
                      />
                    </TableCell>
                    <TableCell className="text-center">
                      <Checkbox
                        ...
                        disabled={b.status !== "confirmed"}
                        onCheckedChange={(v) => handleFlag(b, "permissionSigned", v === true)}
                      />
                    </TableCell>
                    ...
                    <TableCell className="space-x-1 text-right whitespace-nowrap">
                      ...
                      {b.status === "confirmed" ? (
                        <Button size="sm" variant="secondary" onClick={() => handleCancel(b)}>
                          Cancel
                        </Button>
                      ) : (
                        <Button size="sm" variant="outline" onClick={() => handleRestore(b)}>
                          Restore
                        </Button>
                      )}
